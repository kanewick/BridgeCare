...
export const FamilyNotificationIcon: React.FC = () => {
  const navigation = useNavigation();
  const { effectiveTheme } = useAppTheme();
  const { getUnreadMessagesForRole } = useMessageStore();

  const unreadMessages = getUnreadMessagesForRole("family");
  ...
  const handlePress = () => {
    // Navigate to family messages screen
    (navigation as any).navigate("FamilyTabs", { screen: "Messages" });
  };
